...
import { getAllClockings } from "../redux/clocking/clockingAction";
...
const TABLE_HEAD = [
  { id: "user", label: "Agente", alignRight: false },
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(
      array,
      (_clocking) =>
        _clocking.name.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
...
const Clocking = ({
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [getAllClockings, getCurrentProfile,roles]);
