...
// material
import { Stack, TextField, Typography, Button, FormControl, InputLabel, MenuItem, Select } from "@mui/material";
...
import { createAdvertisingPackage } from "../../../redux/advertisingPackage/advertisingPackageAction";
...
const AdvertisingAddForm = ({
  ...
  const AddCompanySchema = Yup.object().shape({
    ...
    details: Yup.string().required("Detalhes é obrigatório"),
  });
      ...
      onSubmit={(values, actions) => {
        try {
          setTimeout(async () => {
            ...
            await createAdvertisingPackage(formData);
            actions.resetForm();
...
AdvertisingAddForm.propTypes = {
  createAdvertisingPackage: PropTypes.func.isRequired,
...
export default connect(mapStateToProps, { createAdvertisingPackage })(
  AdvertisingAddForm
);
