...
const CompanyAddForm = ({
  ...
  useEffect(() => {
    ...
    getAllCompanyCategorys();
    getAllAdvertisingPackage();
  }, [getCurrentProfile, getAllCompanyCategorys, getAllAdvertisingPackage]);
    ...
    <Formik
      ...
      onSubmit={(values, actions) => {
        try {
          ...
          setTimeout(async () => {
            await createCompany(fromData).then(async (result) => {
              if (values.company_activity === "BayQi Publicidade") {
                let advertising = advertisingPackages.filter(
                  (advert) => advert.name === values.advertising
                );
                console.log("result",result.payload[result.payload.length - 1]._id);
                var formData = {
                  companyId: result.payload[result.payload.length - 1]._id,
                ...
                <>
                  <FormControl fullWidth>
                    ...
                    <Select
                      ...
                      onChange={props.handleChange}
                      error={Boolean(
                        props.touched.advertising && props.errors.advertising
                      )}
                      helperText={
                        props.touched.advertising && props.errors.advertising
                      }
